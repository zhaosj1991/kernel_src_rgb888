			...
			struct v4l2_pix_format *pix)
{
	...
	dev_dbg(&chan->video.dev, "__tegra_channel_set_format begin, pix->pixelformat = %d\n", 
		pix->pixelformat);
	vfmt = tegra_core_get_format_by_fourcc(chan, pix->pixelformat);
	if (NULL != vfmt)
		dev_dbg(&chan->video.dev, "tegra_core_get_format_by_fourcc success, vfmt->code = %d\n", vfmt->code);
	...
	ret = v4l2_subdev_call(sd, pad, set_fmt, NULL, &fmt);
	dev_dbg(&chan->video.dev, "v4l2_subdev_call set_fmt ret = %d\n", ret);
	if (ret == -ENOIOCTLCMD)
		return -ENOTTY;
	...
	if (!ret) {
		...
		*pix = chan->format;
		dev_dbg(&chan->video.dev, "chan->format.pixelformat = %d\n", chan->format.pixelformat);
		dev_dbg(&chan->video.dev, "chan->total_ports = %d\n", chan->total_ports);
